...
export function useNotes() {
  ...
  const loadNotes = useCallback(async () => {
    try {
      ...
      setMarkedNotes(
        fetchedNotes.filter((note: Note) => note.is_marked).map((note: Note) => note.id)
      );
    } catch (err) {
      const apiErr = toApiError(err);
      clientLogger.error('Error loading notes', apiErr);
  ...
  const loadTrashNotes = useCallback(async () => {
    try {
      ...
      const apiErr = toApiError(err);
      clientLogger.error('Error loading trash notes', apiErr);
      if (apiErr.status === 401) {
        authService.logout();
        navigate('/login', { replace: true });
      }
  ...
  const handleCreateNote = async () => {
    ...
    try {
      ...
      clientLogger.error('Error creating note', apiErr);
      toast.error(apiErr.message || 'Error al crear la nota');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleUpdateNote = async (id: string, field: 'title' | 'content') => {
    ...
    try {
      ...
      if (response && response.note) {
        setNotes(prevNotes => prevNotes.map(note => (note.id === id ? response.note : note)));

        forceReorder();
        toast.success('Nota actualizada');
      }
    } catch (error) {
      clientLogger.error('Error al actualizar nota', toApiError(error));
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>, noteId: string) => {
    ...
    try {
      ...
      clientLogger.error('Error uploading image', toApiError(error));
  ...
  const handleDeleteImage = async (noteId: string, imageIndex: number) => {
    ...
    try {
      ...
      clientLogger.error('Error deleting image', toApiError(error));
  ...
  const handleDeleteNote = async (id: string) => {
    try {
      ...
      clientLogger.error('Error deleting note', toApiError(error));
  ...
  const handleRestoreNote = async (id: string) => {
    try {
      ...
      clientLogger.error('Error al restaurar nota', toApiError(error));
  ...
  const handleEmptyTrash = async () => {
    ...
    try {
      ...
      clientLogger.error('Error al vaciar papelera', toApiError(error));
  ...
  const handleDeleteMarkedNotes = async () => {
    ...
    if (window.confirm(`¿Estás seguro de que quieres eliminar ${markedNotes.length} nota(s)?`)) {
      try {
        ...
        clientLogger.error('Error al procesar notas', toApiError(error));
  ...
  const handleToggleMark = async (id: string, event: React.MouseEvent) => {
    ...
    try {
      ...
      setMarkedNotes(prev => {
        ...
        return isCurrentlyMarked ? prev.filter(noteId => noteId !== id) : [...prev, id];
      });

      // Llamada a la API sin actualizar el estado de las notas
      await noteService.toggleMark(id);
    } catch (error) {
      clientLogger.error('Error al marcar/desmarcar nota', toApiError(error));
      // Restaurar el estado anterior en caso de error
      setMarkedNotes(prev => {
        const wasMarked = !prev.includes(id);
        return wasMarked ? [...prev, id] : prev.filter(noteId => noteId !== id);
      });
    }
  };
  ...
  const handleTogglePin = async (id: string, event: React.MouseEvent) => {
    ...
    try {
      ...
      clientLogger.error('Error al fijar/desfijar nota', toApiError(error));
      toast.error('Error al actualizar la nota');
    }
  };
