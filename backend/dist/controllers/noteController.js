...
const storage = multer_1.default.diskStorage({
    destination: (req, file, cb) => {
        const uploadDir = path_1.default.join(__dirname, "..", "uploads", "note-images");
...
class NoteController {
    ...
    async deleteNote(req, res) {
        try {
            ...
            if (isInTrash) {
                ...
                await database_1.pool.query("DELETE FROM notes WHERE id = $1 AND user_id = $2", [
                    id,
                    userId,
                ]);
    ...
    async shareNote(req, res) {
        try {
            ...
            const note = await database_1.pool.query("SELECT * FROM notes WHERE id = $1 AND user_id = $2", [noteId, ownerId]);
            if (note.rows.length === 0) {
                res
                    .status(404)
            ...
            const targetUser = await database_1.pool.query("SELECT id FROM users WHERE username = $1", [username]);
            ...
            const existingShare = await database_1.pool.query("SELECT * FROM shared_notes WHERE note_id = $1 AND shared_with_id = $2", [noteId, sharedWithId]);
            if (existingShare.rows.length > 0) {
                // Si ya está compartida, actualizamos los permisos
                await database_1.pool.query("UPDATE shared_notes SET can_edit = $1, include_images = $2, updated_at = CURRENT_TIMESTAMP WHERE note_id = $3 AND shared_with_id = $4", [canEdit, includeImages, noteId, sharedWithId]);
                res.status(200).json({
                    success: true,
            ...
            await database_1.pool.query("INSERT INTO shared_notes (note_id, owner_id, shared_with_id, can_edit, include_images) VALUES ($1, $2, $3, $4, $5)", [noteId, ownerId, sharedWithId, canEdit, includeImages]);
            res
                .status(200)
    ...
    async updateSharedNotePermissions(req, res) {
        try {
            const { id } = req.params;
            ...
            const isOwner = await database_1.pool.query("SELECT 1 FROM shared_notes WHERE note_id = $1 AND owner_id = $2 AND shared_with_id = $3", [id, ownerId, sharedWithId]);
            ...
            await database_1.pool.query(updateQuery, queryParams);
    ...
    async updateSharedNote(req, res) {
        try {
            const { id } = req.params;
            const { title, content, images } = req.body;
            const userId = req.user.id;
            ...
            // Verificar permisos
            const hasPermission = await database_1.pool.query(`SELECT 1 FROM shared_notes 
       WHERE note_id = $1 
       AND shared_with_id = $2 
       AND can_edit = true`, [id, userId]);
    ...
    async searchUsers(req, res) {
        try {
            ...
            const result = await database_1.pool.query(`SELECT id, username FROM users 
       WHERE username ILIKE $1 
       ORDER BY username ASC 
       LIMIT 10`, [`${query}%`] // Busca usuarios que comiencen con la consulta
            );
