...
const storage = multer.diskStorage({
  destination: (req, file, cb) => {
    const uploadDir = path.join(__dirname, '..', 'uploads', 'group-note-images');
...
async getUserGroups(req: Request, res: Response): Promise<void> {
  try {
    ...
    res.status(500).json({ error: 'Error al obtener los grupos', groups: [] }); // Siempre devuelve un array vacío en caso de error
  }
}
  ...
  async updateUserGroup(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (role !== 'owner' && role !== 'admin') {
        res.status(403).json({ error: 'No tienes permisos para actualizar este grupo' });
        return;
      }
  ...
  async deleteUserGroup(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (ownerCheckResult.rows[0].owner_id !== userId) {
        res.status(403).json({ error: 'Solo el propietario puede eliminar el grupo' });
        return;
      }
  ...
  async addGroupMember(req: Request, res: Response): Promise<void> {
    ...
    try {
      ...
      if (currentUserRole !== 'owner' && currentUserRole !== 'admin') {
        res.status(403).json({ error: 'No tienes permisos para añadir miembros' });
        return;
      }
      
      // Verificar que solo el propietario puede añadir administradores
      if (role === 'admin' && currentUserRole !== 'owner') {
        res.status(403).json({ error: 'Solo el propietario puede añadir administradores' });
        return;
      }
  ...
  async removeGroupMember(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (userId !== memberUserId) {
        ...
        const currentUserRole = roleCheckResult.rows[0].role;
        if (currentUserRole !== 'owner' && currentUserRole !== 'admin') {
          res.status(403).json({ error: 'No tienes permisos para eliminar miembros' });
          return;
        }
        ...
        if (currentUserRole === 'admin' && (memberRole === 'owner' || memberRole === 'admin')) {
          res.status(403).json({ error: 'No tienes permisos para eliminar a este miembro' });
          return;
        }
        
        // No se puede eliminar al propietario
        if (memberRole === 'owner') {
          res.status(403).json({ error: 'No se puede eliminar al propietario del grupo' });
          return;
        }
      }
  ...
  async updateMemberRole(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (memberUserId === ownerCheckResult.rows[0].owner_id) {
...
async getGroupNotes(req: Request, res: Response): Promise<void> {
  try {
    ...
    if (!isMember) {
      res.status(403).json({ error: 'No tienes acceso a este grupo', notes: [] });
      return;
    }
    ...
    res.status(500).json({ error: 'Error al obtener las notas del grupo', notes: [] });
  }
}
...
// Verificar que el usuario es el creador de la nota o tiene permisos de administrador/propietario
  ...
  async togglePinGroupNote(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (noteCheckResult.rows[0].user_id !== userId) {
        ...
        const role = roleCheckResult.rows[0].role;
        if (role !== 'owner' && role !== 'admin') {
          res.status(403).json({ error: 'No tienes permisos para modificar esta nota' });
          return;
        }
      }
      ...
      res.json({
        message: updateResult.rows[0].is_pinned ? 'Nota marcada como importante' : 'Nota desmarcada',
        note: getNoteResult.rows[0]
      });
    } catch (error) {
  ...
  async inviteUserByEmail(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (role !== 'owner' && role !== 'admin') {
        res.status(403).json({ error: 'No tienes permisos para invitar usuarios' });
        return;
      }
  ...
  async searchUsers(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (!query || query.trim().length < 2) {
        res.status(400).json({ error: 'La consulta de búsqueda debe tener al menos 2 caracteres' });
        return;
      }
  ...
  async transferOwnership(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (groupResult.rows[0].owner_id !== userId) {
        res.status(403).json({ error: 'Solo el propietario puede transferir la propiedad' });
        return;
      }
      ...
      if (!newOwnerCheckResult.rows[0].is_member) {
        res.status(400).json({ error: 'El nuevo propietario debe ser miembro del grupo' });
        return;
      }
      ...
      res.status(500).json({ error: 'Error al transferir propiedad del grupo' });
    }
  }
}